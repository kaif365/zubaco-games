...
import { Check, Clock3, Layers, Loader2, PlusCircle, Save } from "lucide-react";
import { ScoringSection } from "./ScoringSection";
import type {
  ArrowLevel,
  ArrowStageConfigLevel,
  InfinityGameConfig as InfinityStageConfig,
...
interface InfinityGameConfigProps {
  gameId?: string;
  gameName: string;
  stageId: string;
  config: InfinityStageConfig | null;
...
function areConfigsEquivalent(
  a: InfinityStageConfig,
  b: InfinityStageConfig,
): boolean {
  if (a.timeLimit !== b.timeLimit) return false;
  if ((a.maxTimeBonus ?? 0) !== (b.maxTimeBonus ?? 0)) return false;
  return areLevelCountsEqual(
    sortLevelCounts(a.levels),
    sortLevelCounts(b.levels),
  );
}

function validateConfig(config: InfinityStageConfig): string | null {
...
const FORM_PREFIX = "infinity-loop";

export function InfinityGameConfig({
  gameId,
  gameName,
  stageId,
  config,
}: InfinityGameConfigProps) {
  const [draftValues, setDraftValues] = useState<InfinityStageConfig | null>(
    null,
  );
  const [createBaseline, setCreateBaseline] =
    useState<InfinityStageConfig | null>(null);
  ...
  const updateValues = (
    updater: (source: InfinityStageConfig) => InfinityStageConfig,
  ) => {
  ...
  if (!sortedLevels.length) {
    return (
      <div className="rounded-xl border border-border bg-muted/20 p-6 text-sm text-muted-foreground">
        No Infinity levels were found. Add levels like Easy, Medium, and Hard in
        the Infinity service first.
      </div>
    );
  }
  ...
  if (!values) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-6">
        <div className="space-y-2">
          <p className="text-sm font-medium text-foreground">
            No Infinity configuration exists for this stage yet.
          </p>
        ...
        <div className="space-y-2 border-b border-border pb-4 pt-0 last:border-0">
          <div className="pl-8">
            <p className="text-xs text-muted-foreground">
              Set the Infinity Loop round time. Maximum time must stay at least{" "}
