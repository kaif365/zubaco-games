...
interface SpotTheDifferenceGameConfigProps {
  gameId?: string;
  gameName: string;
  stageId: string;
  config: SpotTheDifferenceConfig | null;
...
function mapLevelsToBoardConfig(
  levels: ArrowLevel[],
  configuredLevels: SpotTheDifferenceStageConfigLevel[] = [],
): SpotTheDifferenceStageConfigLevel[] {
...
function normalizeConfigForComparison(
  config: SpotTheDifferenceConfig,
): SpotTheDifferenceConfig {
  return {
    ...config,
...
function sortLevelCounts(levels: SpotTheDifferenceStageConfigLevel[]) {
  return [...levels].sort((a, b) => a.levelId.localeCompare(b.levelId));
}

function areLevelCountsEqual(
  a: SpotTheDifferenceStageConfigLevel[],
  b: SpotTheDifferenceStageConfigLevel[],
): boolean {
...
function areConfigsEquivalent(
  a: SpotTheDifferenceConfig,
  b: SpotTheDifferenceConfig,
): boolean {
  if (a.timeLimit !== b.timeLimit) return false;
  if (a.hintCount !== b.hintCount) return false;
  if ((a.enableDemo ?? false) !== (b.enableDemo ?? false)) return false;
  if ((a.maxTimeBonus ?? 0) !== (b.maxTimeBonus ?? 0)) return false;

  const aNormalized = normalizeConfigForComparison(a);
  const bNormalized = normalizeConfigForComparison(b);
...
function validateConfig(config: SpotTheDifferenceConfig): string | null {
...
export function SpotTheDifferenceGameConfig({
  gameId,
  gameName,
  stageId,
  config,
}: SpotTheDifferenceGameConfigProps) {
  const [draftValues, setDraftValues] =
    useState<SpotTheDifferenceConfig | null>(null);
  const [createBaseline, setCreateBaseline] =
    useState<SpotTheDifferenceConfig | null>(null);
  const [hasUserEdits, setHasUserEdits] = useState(false);
  const [timerBounds, setTimerBounds] = useState<TimerBounds>(() =>
    buildTimerBounds(config?.timeLimit ?? 300),
  );
  const [saveState, setSaveState] = useState<SaveState>('idle');
  const { data: gameLevels, isLoading: levelsLoading } =
    useGameLevels(gameName);
  const { mutateAsync } = useSaveGameConfig(gameId, gameName);
  const { toast } = useToast();

  const sortedLevels = useMemo(
    () => sortLevels(gameLevels ?? []),
    [gameLevels],
  );
  ...
  const scoringRules = [
    { label: 'Hint Penalty', value: 'maxScore × hints_used / total_differences' },
    { label: 'Final Score', value: 'max(0, base − penalty)' },
  ];
  ...
  const updateValues = (
    updater: (source: SpotTheDifferenceConfig) => SpotTheDifferenceConfig,
  ) => {
  ...
  const handleCreate = () => {
    ...
    setTimerBounds(buildTimerBounds(300));
  ...
  const handleSave = async () => {
    ...
    const message = validateConfig(payloadValues);
    if (message) {
      toast({
  ...
  if (!sortedLevels.length) {
    return (
      <div className="rounded-xl border border-border bg-muted/20 p-6 text-sm text-muted-foreground">
        No Spot the Difference levels were found. Add levels like Easy, Medium, and Hard in the Spot the Difference service first.
      </div>
    );
  }
  ...
  if (!values) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-6">
        <div className="space-y-2">
          <p className="text-sm font-medium text-foreground">
            No Spot the Difference configuration exists for this stage yet.
          </p>
          <p className="text-sm text-muted-foreground">
            Create one here by setting a time limit, maximum hints allowed, and level board counts.
          </p>
        </div>
    ...
    <div className="space-y-3">
        ...
        <div className="space-y-2 border-b border-border pb-4 pt-0 last:border-0">
          <div className="pl-8">
            <p className="text-xs text-muted-foreground">
              Set the Spot the Difference round time. Maximum time must stay at least{' '}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label
                htmlFor={`${stageId || 'std'}-timer-max`}
                className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground"
              >
                Maximum Time
              </Label>
              <Input
                id={`${stageId || 'std'}-timer-max`}
                type="number"
      ...
      >
        <div className="flex items-center justify-between py-2">
          <div>
            <p className="text-sm font-medium text-foreground">Available Hints</p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              Maximum hints players can use during a Spot the Difference round.
            </p>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <Stepper
              value={values.hintCount}
              min={HINT_LIMITS.min}
              max={HINT_LIMITS.max}
              onChange={(hintCount) =>
                updateValues((source) => ({ ...source, hintCount }))
              }
            />
          </div>
        </div>
