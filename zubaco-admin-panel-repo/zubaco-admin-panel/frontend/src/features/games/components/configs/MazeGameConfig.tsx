...
interface MazeGameConfigProps {
  gameId?: string;
  gameName: string;
  stageId: string;
  config: MazeConfig | null;
...
function sortLevels(levels: MazeLevel[]): MazeLevel[] {
  return [...levels].sort((a, b) => compareLevelsByName(a.name, b.name));
}

function isDemoLevel(level: MazeLevel): boolean {
...
function mapLevelsToBoardConfig(
  levels: MazeLevel[],
  configuredLevels: MazeStageConfigLevel[] = [],
): MazeStageConfigLevel[] {
...
function normalizeMazeConfigForComparison(config: MazeConfig): MazeConfig {
  return {
    ...config,
...
function sortLevelCounts(levels: MazeStageConfigLevel[]) {
  return [...levels].sort((a, b) => a.levelId.localeCompare(b.levelId));
}

function areLevelCountsEqual(
  a: MazeStageConfigLevel[],
  b: MazeStageConfigLevel[],
): boolean {
...
function areMazeConfigsEquivalent(a: MazeConfig, b: MazeConfig): boolean {
  ...
  const aNormalized = normalizeMazeConfigForComparison(a);
  const bNormalized = normalizeMazeConfigForComparison(b);
...
function validateMazeConfig(config: MazeConfig): string | null {
...
export function MazeGameConfig({
  gameId,
  gameName,
  stageId,
  config,
}: MazeGameConfigProps) {
  const [draftValues, setDraftValues] = useState<MazeConfig | null>(null);
  const [createBaseline, setCreateBaseline] = useState<MazeConfig | null>(
    null,
  );
  ...
  const scoringRules = [
    { label: "Inefficient Path", value: "floor(maxScore × optimal / actual steps)" },
    { label: "Incomplete Maze", value: "0 pts" },
  ...
  const isDirty =
    hasUserEdits && values
      ? dirtyBaseline
        ? !areMazeConfigsEquivalent(values, dirtyBaseline)
        : true
      : false;

  const validationMessage = values ? validateMazeConfig(values) : null;
  const isValid = !validationMessage;

  const updateValues = (updater: (source: MazeConfig) => MazeConfig) => {
  ...
  const handleSave = async () => {
    ...
    const message = validateMazeConfig(payloadValues);
    if (message) {
      toast({
  ...
  if (!sortedLevels.length) {
    return (
      <div className="rounded-xl border border-border bg-muted/20 p-6 text-sm text-muted-foreground">
        No Maze levels were found. Add levels like Easy, Medium, and Hard in
        the Maze service first.
      </div>
    );
  }
  ...
  if (!values) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-6">
        <div className="space-y-2">
          <p className="text-sm font-medium text-foreground">
            No Maze configuration exists for this stage yet.
          </p>
        ...
        <div className="space-y-2 border-b border-border pb-4 pt-0 last:border-0">
          <div className="pl-8">
            <p className="text-xs text-muted-foreground">
              Set the Maze round time. Maximum time must stay at least{" "}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label
                htmlFor={`${stageId || "maze"}-timer-max`}
                className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground"
              >
                Maximum Time
              </Label>
              <Input
                id={`${stageId || "maze"}-timer-max`}
                type="number"
      ...
      <SectionCard icon={<BookOpen className="h-3.5 w-3.5" />} title="Demo">
        <div className="flex items-center justify-between border-b border-border last:border-none py-4">
          ...
          <Switch
            ...
            aria-label="Toggle maze demo mode"
          />
        </div>
