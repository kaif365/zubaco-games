...
export async function importKeyFromHex(hexKey: string): Promise<CryptoKey> {
  ...
  const key = await crypto.subtle.importKey(
    'raw',
    bytesToBufferSource(hexToBytes(hexKey)),
    { name: 'AES-GCM' },
    false,
    ['encrypt', 'decrypt'],
  );
...
export async function encryptPayload(data: unknown, hexKey: string): Promise<EncryptedPayload> {
  ...
  const encrypted = await crypto.subtle.encrypt(
    { name: 'AES-GCM', iv: bytesToBufferSource(iv), tagLength: 128 },
    key,
    bytesToBufferSource(stringToBytes(JSON.stringify(data))),
  );
