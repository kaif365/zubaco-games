...
const STORAGE_KEY = 'logic-reflector-stats';
...
export function updateStats(updates: Partial<{
  ...
  if (updates.bestMoves !== undefined) stats.bestMoves = Math.max(stats.bestMoves, updates.bestMoves);
  ...
  if (updates.perfectGames !== undefined) stats.perfectGames = updates.perfectGames;
...
const STAT_DISPLAY = [
  ...
  { key: 'totalWins', label: 'Puzzles Solved', icon: '✅' },
  { key: 'totalMoves', label: 'Total Moves', icon: '🪞' },
  { key: 'bestMoves', label: 'Fewest Moves', icon: '🎯' },
  { key: 'maxStreak', label: 'Best Streak', icon: '🔥' },
  { key: 'highestLevel', label: 'Highest Level', icon: '📐' },
  { key: 'perfectGames', label: 'Perfect Solves', icon: '💎' },
  { key: 'fastCompletions', label: 'Speed Solves', icon: '⚡' },
...
export function StatsScreen({ onBack }: StatsScreenProps) {
  ...
  function formatValue(key: string, value: number, format?: string): string {
    if (format === 'time') {
          ...
          {STAT_DISPLAY.map((stat, i) => (
            ...
            >
              ...
              <p className="text-lg font-bold text-white">
                {formatValue(stat.key, (stats as Record<string, number>)[stat.key] ?? 0, stat.format)}
              </p>
