...
import { stageContentApi } from '@/features/flow-puzzle/api/stageContent.api';
import { stageContentKeys } from '@/features/flow-puzzle/api/stageContent.keys';
...
export function useStageContent({ enabled = true }: UseStageContentOptions = {}): UseStageContentResult {
  ...
  const stageIdNum = Number(appEnv.userStageNumber);
  const stageNumber: StageId = (stageIdNum >= 1 && stageIdNum <= 7 ? stageIdNum : 1) as StageId;
  const adminBaseUrl = import.meta.env.VITE_ADMIN_API_BASE_URL;
  const lang = i18n.language.toUpperCase() === 'HI' ? 'HI' : 'EN';

  const shouldFetch = Boolean(adminBaseUrl) && Boolean(stageId) && enabled;
