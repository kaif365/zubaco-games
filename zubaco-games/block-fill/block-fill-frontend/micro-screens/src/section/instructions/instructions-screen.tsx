...
export function GameInstructionsScreen({
  ...
  const listRefs = useRef<Array<HTMLOListElement | null>>([]);
  const badgeRefs = useRef<Array<Array<HTMLSpanElement | null>>>([]);
  const [connectorSegments, setConnectorSegments] = useState<
    Array<Array<{ top: number; height: number }>>
  >([]);

  const content = useMemo(() => {
    // Resolve base content: backend override takes priority, then i18n, then static map
  ...
  useLayoutEffect(() => {
    const measure = () => {
      const allSegments = listRefs.current.map((listEl, slideIndex) => {
        if (!listEl) return [];

        const listRect = listEl.getBoundingClientRect();
        const badges = badgeRefs.current[slideIndex] || [];
        ...
        for (let i = 0; i < badges.length - 1; i++) {
          ...
          segments.push({
            top: currentRect.bottom - listRect.top + 2,
            height: Math.max(nextRect.top - currentRect.bottom - 4, 0),
          });
        }
      ...
      setConnectorSegments(allSegments);
    };
  ...
  return (
    <div className="instructionViewport">
      ...
      >
        <img src={overlay} alt="" aria-hidden className="overlay" loading="eager" sizes="100vw" />
        <div
          className="eclipse"
        ...
        <div className="content">
                    ...
                    <div className="card">
                      ...
                      >
                        <ol
                          ...
                          ref={(el) => {
                            listRefs.current[slideIndex] = el;
                          }}
                        >
                          {connectorSegments[slideIndex]?.map((segment, index) => (
                            <span
                              key={`${index}-connector`}
                              className="connector"
                              aria-hidden
                            ...
                            return (
                              <li className={`item ${isRule ? 'items-center' : ''}`} key={item.id}>
                                <span
                                  ...
                                  ref={(el) => {
                                    if (!badgeRefs.current[slideIndex]) {
                                      badgeRefs.current[slideIndex] = [];
                                    }

                                    badgeRefs.current[slideIndex][index] = el;
                                  }}
                                >
                                  {isRule ? '×' : index + 1}
                                </span>
          ...
          <div className="ctaStack">
            <button
              ...
              type="button"
              onClick={isPlayNowLoading ? undefined : () => onPlayNow?.(stage)}
            ...
            >
              {isPlayNowLoading ? (
                <span className="buttonLoadingContent">
                  ...
                  {t('instructions.labels.starting')}
                </span>
              ) : (
                t('instructions.labels.playNow')
              )}
            </button>
            {!hideLearnHowToPlay && (
              <button
              ...
              >
                {t('instructions.labels.learnHowToPlay')}
              </button>
            )}
          </div>
        </div>
