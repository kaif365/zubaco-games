...
export class LevelService {
    ...
    /**
     * Creates a new level if no active level already uses the same name.
     * @param {CreateLevelDto} dto - The payload containing the level details.
     * @returns {Promise<void>} A promise that resolves when the level is created.
     */
     ...
     * @param {UpdateLevelDto} dto - The payload containing the level identifier and new name.
    ...
    /**
     * Soft-deletes levels that are not referenced by active stage configs.
     * @param {DeleteLevelsDto} dto - The payload containing the level identifiers to delete.
     * @returns {Promise<void>} A promise that resolves when the levels are deleted.
     */
    async remove(dto: DeleteLevelsDto) {
        ...
        // Check no active stage configs reference these levels
        const [stageConfigUsage, demoStageConfigUsage] = await Promise.all([
        ...
        if (stageConfigUsage || demoStageConfigUsage) {
    ...
    /**
     * Lists active levels using search and pagination filters.
     * @param {ListLevelsDto} dto - The query payload containing paging and filter values.
     * @returns {Promise<{ data: unknown[]; totalCount: number }>} The paginated level list.
     */
    async list(dto: ListLevelsDto) {
        ...
        const [data, totalCount] = await Promise.all([
            this.prisma.level.findMany({
                ...
                select: { id: true, name: true, difficultyScore: true, createdAt: true },
            }),
