...
export class GameController {
   ...
   * @param {StartGameDto} dto - dto value.
   *
  ...
  gameStart(@CurrentUser() user: GameUser, @Body() dto: StartGameDto) {
    return this.gameSessionRestate.startGame(
      user.id,
      user.stageId,
      dto.clientSeed ?? crypto.randomUUID(),
    );
  }
  ...
  nextBoard(@CurrentUser() user: GameUser) {
    return this.gameSessionRestate.nextBoard(user.id, user.stageId);
  }
