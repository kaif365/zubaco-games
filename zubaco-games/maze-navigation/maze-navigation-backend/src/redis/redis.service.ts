...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async setSession(
    ...
    await this.client.set(
      `${config.redis.mazeProjectKey}:session:${sessionId}`,
      JSON.stringify(data),
      "EX",
      ttlSeconds,
    );
  ...
  async setUser(
    ...
    const key = `${config.redis.mazeProjectKey}:user:${userId}`;
    if (ttlSeconds) {
  ...
  async getUser(userId: string): Promise<RedisUser | null> {
    const raw = await this.client.get(
      `${config.redis.mazeProjectKey}:user:${userId}`,
    );
    if (!raw) {
      return null;
    }
  ...
  async deleteUser(userId: string): Promise<void> {
    await this.client.del(`${config.redis.mazeProjectKey}:user:${userId}`);
  }
   ...
   * @param {string} key - key value.
   *
   * @returns {Promise<string | null>} The asynchronous result.
   */
