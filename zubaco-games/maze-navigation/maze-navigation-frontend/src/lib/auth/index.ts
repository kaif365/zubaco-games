...
export const clearSessionToken = clearSessionTokenStorage;
export const clearAuthStorage = clearSessionTokenStorage;

export function isDevSessionRequestUrl(url: string | undefined): boolean {
  if (!url) {
    return false;
  }
  return url.toLowerCase().includes("dev-session");
}
