...
const STORAGE_KEY = 'reflex-endurance-stats';
...
export function StatsScreen({ onBack }: StatsScreenProps) {
  ...
  const statCards = [
    ...
    { label: 'Correct Taps', value: stats.correctTaps.toLocaleString(), icon: '✅' },
    { label: 'Wrong Taps', value: stats.wrongTaps.toLocaleString(), icon: '❌' },
    { label: 'Total Taps', value: stats.totalTaps.toLocaleString(), icon: '👆' },
    { label: 'Best Streak', value: stats.bestStreak.toLocaleString(), icon: '🔥' },
    { label: 'High Score', value: stats.highScore.toLocaleString(), icon: '🏅' },
    { label: 'Avg Reaction', value: stats.avgReactionMs > 0 ? `${Math.round(stats.avgReactionMs)}ms` : '—', icon: '⚡' },
...
export function updateStats(correctTaps: number, wrongTaps: number, streak: number, score: number, timeMs: number, level: number) {
  const stats = loadStats();
  stats.gamesPlayed += 1;
  stats.correctTaps += correctTaps;
  stats.wrongTaps += wrongTaps;
  stats.totalTaps += correctTaps + wrongTaps;
