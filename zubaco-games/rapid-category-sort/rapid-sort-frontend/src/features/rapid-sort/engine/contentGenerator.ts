import type { CategoryItem, CategoryPair, SortDirection } from '@/types/game';
import { mulberry32, seededShuffle } from './random';

/**
 * Word bank: each entry is [categoryA, categoryB, itemsForA[], itemsForB[]]
 * Items for A go LEFT, items for B go RIGHT
 */
...
/**
 * Generate the full item sequence for a game session.
 * Picks category pairs, then interleaves items from them.
 */
export function generateItemSequence(
  ...
  for (let pairIdx = 0; pairIdx < selectedPairs.length; pairIdx++) {
    ...
    // Randomly swap left/right assignment for variety
  ...
  // Shuffle all items and take totalItems
...
/**
 * Get the category pair for a given item (to display correct labels).
 * In practice, we cycle through pairs — each batch of items shows one pair's labels.
 */
export function getCategoryForItem(
  ...
  const itemsPerPair = Math.ceil(totalItems / pairs.length);
  const pairIdx = Math.min(Math.floor(itemIndex / itemsPerPair), pairs.length - 1);
