...
const STORAGE_KEY = "infinity-loop-stats";
...
export function updateStats(update: Partial<GameStats> & { addMoves?: number; addTime?: number }): GameStats {
  ...
  if (update.addMoves) stats.totalMoves += update.addMoves;
  if (update.bestMoves !== undefined && (stats.bestMoves === null || update.bestMoves < stats.bestMoves)) {
    stats.bestMoves = update.bestMoves;
  }
...
export function StatsScreen({ onBack }: StatsScreenProps) {
  ...
  const statItems = [
    ...
    { label: "Best Moves", value: stats.bestMoves ?? "—", icon: "🎯" },
    ...
    },
    {
      ...
      value: stats.totalGamesPlayed > 0 ? Math.round(stats.totalMoves / stats.totalGamesPlayed) : "—",
      icon: "📊",
    },
  ];
