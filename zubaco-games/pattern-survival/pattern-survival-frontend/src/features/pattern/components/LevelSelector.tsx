...
const STORAGE_KEY = 'pattern-survival-levels';
...
export function LevelSelector({ onSelect, onBack }: Props) {
  ...
  const configs = ['800ms', '700ms', '650ms', '600ms', '550ms', '500ms', '450ms', '400ms', '350ms', '300ms'];
        ...
        {levels.map((lvl, i) => (
          <motion.button
            ...
            className={`aspect-square rounded-xl flex flex-col items-center justify-center text-sm font-bold border-2 transition-colors
              ${lvl.unlocked ? 'bg-purple-700 border-purple-500 hover:bg-purple-600' : 'bg-gray-800 border-gray-700 opacity-50'}`}
          >
            {lvl.unlocked ? (
              <>
                <span className="text-lg">{i + 1}</span>
                <span className="text-[9px] text-gray-300">{configs[i]}</span>
