...
export class RedisService implements OnModuleInit, OnModuleDestroy {
    ...
    async setSession(sessionId: string, data: RedisSession, ttlSeconds: number): Promise<void> {
        await this.client.set(
            `${config.redis.slidingPuzzleProjectKey}:session:${sessionId}`,
            JSON.stringify(data),
            'EX',
            ttlSeconds,
        );
    ...
    async deleteSession(sessionId: string): Promise<void> {
        await this.client.del(`${config.redis.slidingPuzzleProjectKey}:session:${sessionId}`);
    }
    ...
    async setUser(userId: string, data: RedisUser, ttlSeconds?: number): Promise<void> {
        const key = `${config.redis.slidingPuzzleProjectKey}:user:${userId}`;
        if (ttlSeconds) {
    ...
    async getUser(userId: string): Promise<RedisUser | null> {
        const raw = await this.client.get(`${config.redis.slidingPuzzleProjectKey}:user:${userId}`);
        if (!raw) {
            return null;
        }
    ...
    async deleteUser(userId: string): Promise<void> {
        await this.client.del(`${config.redis.slidingPuzzleProjectKey}:user:${userId}`);
    }
