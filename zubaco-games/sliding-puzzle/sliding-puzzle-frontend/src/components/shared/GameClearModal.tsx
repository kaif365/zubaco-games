...
/**
 * Alert-style completion banner with auto-redirect countdown.
 * Replicates the exact Alert component structure — same layout, spacing,
 * and slide-in animation — but uses theme colors and a Timer icon.
 * Copy-paste ready: only depends on React, react-dom, and lucide-react.
 */
export function GameClearModal({
  ...
  // Slide-in animation — mirrors Alert's requestAnimationFrame pattern
  useEffect(() => {
    const raf = requestAnimationFrame(() => { setIsAnimated(true); });
    return () => cancelAnimationFrame(raf);
  }, []);

  // Countdown — fires onConfirm when it reaches 0
  useEffect(() => {
    if (seconds <= 0) {
  ...
  return createPortal(
    <div className="fixed left-1/2 bottom-[max(18px,env(safe-area-inset-bottom))] z-[100] w-[calc(100vw-32px)] max-w-[520px] -translate-x-1/2 px-4">
      {/* Exact same structure as Alert — wrapper, icon circle, text block */}
      <div
