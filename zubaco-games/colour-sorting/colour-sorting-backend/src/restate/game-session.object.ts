...
function assertSession(session: InFlightSession | null): asserts session is InFlightSession {
  if (!session) throw terminalError('GAME_SESSION_NOT_FOUND', 404);
}
  ...
  return restate.object({
    ...
    handlers: {
      startGame: async (ctx: restate.ObjectContext, req: StartGameRequest) => {
        // Check for existing in-flight session (reconnect)
        ...
        const result = await ctx.run('create-session', () =>
          gameService.startGame(req.userId, req.stageId),
        );
        ...
        // Schedule durable expiry via external service
