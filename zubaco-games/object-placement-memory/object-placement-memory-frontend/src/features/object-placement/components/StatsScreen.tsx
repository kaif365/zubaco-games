...
const STORAGE_KEY = 'object-placement-stats';
...
function loadStats(): PlacementStats {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) return JSON.parse(raw);
  } catch { /* ignore */ }
  return { gamesPlayed: 0, gamesWon: 0, totalCorrect: 0, totalObjects: 0, perfectGames: 0, bestStreak: 0, highestLevel: 0, fastGames: 0, totalTimeSec: 0, highScore: 0     recentScores: [],
  };
}

export function updateStats(patch: Partial<PlacementStats> & { addTimeSec?: number }) {
...
export function StatsScreen({ onBack }: Props) {
  const [stats] = useState<PlacementStats>(loadStats);
  ...
  const accuracy = stats.totalObjects > 0 ? Math.round((stats.totalCorrect / stats.totalObjects) * 100) : 0;
  ...
  const items = [
    ...
    { label: 'Correct Placements', value: stats.totalCorrect.toLocaleString(), icon: '✓' },
