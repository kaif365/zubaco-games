...
type CachedAuth = { kind: 'user'; data: UserData } | { kind: 'admin'; data: AdminData };
 ...
 */
export function setCachedUser(token: string, data: UserData, expSec: number): void {
 ...
 * @returns {void} Resolves when the operation completes.
 */
export function setCachedAdmin(token: string, data: AdminData, expSec: number): void {
