...
import type {
  BlockFillGameConfig as BlockFillStageConfig,
  ArrowLevel,
...
interface BlockFillGameConfigProps {
  gameId?: string;
  gameName: string;
  stageId: string;
  config: BlockFillStageConfig | null;
...
function areConfigsEquivalent(
  a: BlockFillStageConfig,
  b: BlockFillStageConfig,
): boolean {
...
function validateConfig(config: BlockFillStageConfig): string | null {
...
const FORM_PREFIX = "block-fill";

export function BlockFillGameConfig({
  gameId,
  gameName,
  stageId,
  config,
}: BlockFillGameConfigProps) {
  const [draftValues, setDraftValues] = useState<BlockFillStageConfig | null>(
    null,
  );
  const [createBaseline, setCreateBaseline] =
    useState<BlockFillStageConfig | null>(null);
  ...
  const scoringRules = [
    { label: "Demo Board", value: "0 pts" },
    { label: "End Bonus", value: "floor(maxTimeBonus × remaining / total)" },
  ];
  ...
  const updateValues = (
    updater: (source: BlockFillStageConfig) => BlockFillStageConfig,
  ) => {
  ...
  if (!sortedLevels.length) {
    return (
      <div className="rounded-xl border border-border bg-muted/20 p-6 text-sm text-muted-foreground">
        No Block Fill levels were found. Add levels like Easy, Medium, and Hard
        in the Block Fill service first.
      </div>
    );
  }
  ...
  if (!values) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-muted/20 p-6">
        <div className="space-y-2">
          <p className="text-sm font-medium text-foreground">
            No Block Fill configuration exists for this stage yet.
          </p>
  ...
  const fieldIdPrefix = `${stageId || FORM_PREFIX}`;
        ...
        <div className="space-y-2 border-b border-border pb-4 pt-0 last:border-0">
          <div className="pl-8">
            <p className="text-xs text-muted-foreground">
              Set the Block Fill round time. Maximum time must stay at least{" "}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              <Label
                htmlFor={`${fieldIdPrefix}-timer-max`}
                className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground"
              >
                Maximum Time
              </Label>
              <Input
                id={`${fieldIdPrefix}-timer-max`}
                type="number"
      ...
      <SectionCard icon={<BookOpen className="h-3.5 w-3.5" />} title="Demo">
        <div className="flex items-center justify-between border-b border-border py-4 last:border-none">
          ...
          <Switch
            ...
            aria-label="Toggle Block Fill demo mode"
          />
        </div>
