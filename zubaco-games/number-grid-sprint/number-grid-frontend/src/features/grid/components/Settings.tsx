...
const STORAGE_KEY = 'number-grid-settings';
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm space-y-5">
        ...
        <div className="flex items-center justify-between bg-gray-800 rounded-xl px-4 py-3">
          ...
          <button onClick={() => update({ soundEnabled: !settings.soundEnabled })}
            className={`w-12 h-6 rounded-full relative transition-colors ${settings.soundEnabled ? 'bg-blue-500' : 'bg-gray-600'}`}>
        ...
        <div className="flex items-center justify-between bg-gray-800 rounded-xl px-4 py-3">
          ...
          <button onClick={() => update({ darkMode: !settings.darkMode })}
            className={`w-12 h-6 rounded-full relative transition-colors ${settings.darkMode ? 'bg-blue-500' : 'bg-gray-600'}`}>
