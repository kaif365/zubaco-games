...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Memory Groups API')
    .setDescription('ZUBACO Gaming Engine - Memory Groups Microservice')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  console.log(`Memory Groups backend running on port ${appConfig.port} | API: /api/v1`);
}
bootstrap();
