...
/**
 * Statement bank: [text, isTrue]
 * Covers general knowledge, science, geography, nature, math, common sense
 */
...
/**
 * Generate statement sequence for a game session using PRNG.
 */
export function generateStatements(seed: number, count: number): Statement[] {
  ...
  return selected.map(([text, isTrue], idx) => ({
    id: idx,
    text,
    isTrue,
  }));
