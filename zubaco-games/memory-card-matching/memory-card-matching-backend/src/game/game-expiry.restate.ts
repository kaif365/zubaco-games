    ...
    handlers: {
      markResultProcessing: async (
        ...
        const client = ctx.objectClient(
          ...
          request.objectKey,
        ) as unknown as GameSessionObjectClient;

        await client.markResultProcessing({ sessionId: request.sessionId });
      },
      ...
      finalizeExpiredSession: async (
        ...
        await client.finalizeExpired({ sessionId: request.sessionId });
      },
    },
  });
}

export type GameExpiryRestateService = ReturnType<
  typeof createGameExpiryRestateService
>;
