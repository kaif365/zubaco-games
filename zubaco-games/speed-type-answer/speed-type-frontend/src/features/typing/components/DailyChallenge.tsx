...
const STORAGE_KEY = 'speed-type-daily';
...
function getWeekDays(): { dateStr: string; label: string }[] {
  const now = new Date();
  ...
  for (let i = 0; i < 7; i++) {
    ...
    days.push({ dateStr: getDateStr(d), label: labels[i] });
  }
  return days;
}
...
export function DailyChallenge({ onBack, onStartDaily }: DailyChallengeProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 bg-gradient-to-b from-gray-900 via-gray-900 to-gray-950">
      ...
      <p className="text-center text-gray-400 text-sm mb-6">Test your trivia speed daily!</p>
          ...
          return (
            <motion.div
              ...
              className={`flex flex-col items-center gap-1 p-2 rounded-lg min-w-[40px] ${
                isToday ? 'border-2 border-indigo-500 bg-indigo-500/10' : 'bg-white/5'
              }`}
      ...
      <div className="flex justify-center gap-8 mb-8">
        <div className="text-center">
          <div className="text-2xl font-bold text-indigo-400">{daily.currentStreak}</div>
      ...
      <div className="flex justify-center">
        ...
        >
          {isCompletedToday ? '✓ Completed Today' : 'Start Daily Challenge'}
        </motion.button>
      </div>
    </div>
  );
