...
const STORAGE_PREFIX = "infinity-loop-daily";
...
export function getDailyLevel(): number {
  ...
  // Sunday=7 (hardest), Mon=3, Tue=4, ... Sat=8
...
export function DailyChallenge({ onPlay, onBack }: DailyChallengeProps) {
  ...
  // Generate last 7 days completion status
  const weekDays = Array.from({ length: 7 }, (_, i) => {
    const date = new Date();
    date.setDate(date.getDate() - (6 - i));
    const key = date.toISOString().slice(0, 10);
    const dayCompleted = typeof window !== "undefined"
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-slate-950 p-6">
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-sm mx-auto w-full">
          ...
          {weekDays.map((day) => (
            <div
              ...
              className={`flex flex-col items-center rounded-lg p-2 min-w-[40px] ${
                day.isToday ? "bg-slate-800 border border-emerald-500/40" : ""
              }`}
            >
        ...
        >
          ...
          {completed ? (
            <div className="rounded-xl bg-emerald-900/30 border border-emerald-500/30 p-4">
              ...
              <p className="text-xs text-slate-400 mt-1">Come back tomorrow for a new challenge</p>
            </div>
          ) : (
            <button
              onClick={onPlay}
        ...
        >
          Daily challenges get harder as the week progresses.
          <br />
