...
} from '@/micro-screens/types/instruction-content';
import type { StageId } from '@/micro-screens/types/stage-theme';
...
// Only slides come from the backend — all labels stay in the frontend.
function mapToStageContent(data: StageContentData): Pick<StageContent, 'slides'> | null {
...
function buildContentByStage(
  stageNumber: StageId,
  content: Pick<StageContent, 'slides'>,
...
export function useStageContent(): UseStageContentResult {
  ...
  const adminBaseUrl = import.meta.env.VITE_ADMIN_API_BASE_URL as string | undefined;
