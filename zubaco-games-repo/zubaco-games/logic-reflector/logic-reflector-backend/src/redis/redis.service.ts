...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async setSession(
    ...
    await this.client.set(
      `${config.redis.reflectorProjectKey}:session:${sessionId}`,
      JSON.stringify(data),
      "EX",
      ttlSeconds,
    );
  ...
  async setUser(
    ...
    const key = `${config.redis.reflectorProjectKey}:user:${userId}`;
    if (ttlSeconds) {
  ...
  async getUser(userId: string): Promise<RedisUser | null> {
    const raw = await this.client.get(
      `${config.redis.reflectorProjectKey}:user:${userId}`,
    );
    if (!raw) {
      return null;
    }
  ...
  async deleteUser(userId: string): Promise<void> {
    await this.client.del(`${config.redis.reflectorProjectKey}:user:${userId}`);
  }
