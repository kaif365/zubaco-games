...
const STORAGE_KEY = 'pattern-survival-daily';
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm space-y-5">
        <div className="grid grid-cols-7 gap-2">
          {week.map((date, i) => {
            ...
            return (
              <div key={date} className={`flex flex-col items-center p-2 rounded-lg ${isToday ? 'border-2 border-purple-500 bg-purple-900/30' : 'bg-gray-800'}`}>
        ...
        <div className="bg-gray-800 rounded-xl px-4 py-3 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">Survive as many rounds as possible!</p>
        </div>
        ...
        <button onClick={onPlay} disabled={completedToday}
          className={`w-full py-3.5 rounded-xl font-bold text-lg shadow-lg transition-colors
            ${completedToday ? 'bg-gray-700 text-gray-400' : 'bg-purple-600 hover:bg-purple-500 text-white'}`}>
          {completedToday ? '✓ Completed Today' : 'Start Daily Pattern'}
        </button>
      </motion.div>
    </div>
  );
