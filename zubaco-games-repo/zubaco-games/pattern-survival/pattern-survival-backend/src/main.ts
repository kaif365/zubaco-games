...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Pattern Survival API')
    .setDescription('ZUBACO Gaming Engine - Pattern Survival Microservice')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  console.log(`Pattern Survival backend running on port ${appConfig.port} | API: /api/v1`);
}
bootstrap();
