...
import { buildDefaultStageConfig, DefaultStageConfigOptions } from './stage-config.default';
...
export class StageConfigEventsService implements OnModuleInit {
    ...
    private async handle(message: Message): Promise<void> {
        ...
        if (eventType === GAME_CONFIG_EVENT_TYPE.STAGE_ATTACHED) {
            await this.handleAttached(stageId, {
                ...
                enableNumbers: body.game_config?.enable_numbers ?? undefined,
            });
        } else {
    ...
    private async handleAttached(
        ...
        const defaults = await buildDefaultStageConfig(stageId, this.prisma, options);
        if (!defaults) {
        ...
        const stageConfig = await this.prisma.stageConfig.create({
            data: {
                ...
                maxTimeBonus: defaults.maxTimeBonus,
                enableDemo: defaults.enableDemo,
                enableNumbers: defaults.enableNumbers,
                isEnabled: true,
            },
        });
