export interface LeaderboardEntry { userId: string; score: number; rank: number; achievedAt: string; }
export interface LeaderboardConfig { maxEntries: number; timeWindowMs: number | null; }
const DEFAULT_CONFIG: LeaderboardConfig = { maxEntries: 100, timeWindowMs: null };
export class Leaderboard {
  private entries: LeaderboardEntry[] = [];
  private readonly config: LeaderboardConfig;
  constructor(config: Partial<LeaderboardConfig> = {}) { this.config = { ...DEFAULT_CONFIG, ...config }; }
  ...
  getTop(count: number = 10): LeaderboardEntry[] { return this.entries.slice(0, count); }
  getRank(userId: string): LeaderboardEntry | null { return this.entries.find((e) => e.userId === userId) || null; }
  ...
  private rerank() { this.entries.forEach((e, i) => (e.rank = i + 1)); }
}
