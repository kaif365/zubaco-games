...
const STORAGE_KEY = 'sliding-puzzle-daily';
...
export function DailyChallenge({ onBack, onStartDaily }: DailyChallengeProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 bg-gradient-to-b from-gray-900 via-gray-900 to-gray-950">
      ...
      <p className="text-center text-gray-400 text-sm mb-6">A unique puzzle every day!</p>
          ...
          return (
            <motion.div
              ...
              className={`flex flex-col items-center gap-1 p-2 rounded-lg min-w-[40px] ${
                isToday ? 'border-2 border-amber-500 bg-amber-500/10' : 'bg-white/5'
              }`}
      ...
      <div className="flex justify-center gap-8 mb-8">
        <div className="text-center">
          <div className="text-2xl font-bold text-amber-400">{daily.currentStreak}</div>
          <div className="text-xs text-gray-400">Current Streak</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-rose-400">{daily.bestStreak}</div>
      ...
      <div className="flex justify-center">
        ...
        >
          {isCompletedToday ? '✓ Completed Today' : 'Start Daily Puzzle'}
        </motion.button>
      </div>
    </div>
  );
