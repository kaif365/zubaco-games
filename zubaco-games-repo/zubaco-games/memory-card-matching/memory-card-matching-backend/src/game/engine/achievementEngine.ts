export enum AchievementId { FIRST_WIN = 'FIRST_WIN', STREAK_5 = 'STREAK_5', STREAK_10 = 'STREAK_10', PERFECT_GAME = 'PERFECT_GAME', SPEED_DEMON = 'SPEED_DEMON', PERSISTENCE = 'PERSISTENCE', LEVEL_5 = 'LEVEL_5', LEVEL_10 = 'LEVEL_10', HIGH_SCORER = 'HIGH_SCORER', NO_MISTAKES = 'NO_MISTAKES' }
export interface Achievement { id: AchievementId; title: string; description: string; icon: string; xp: number; }
export interface AchievementProgress { id: AchievementId; unlocked: boolean; unlockedAt: string | null; }
export interface GameStats { totalGamesPlayed: number; totalWins: number; maxStreak: number; highestScore: number; highestLevel: number; perfectGames: number; fastCompletions: number; }
...
export function getAllAchievements(): Achievement[] { return ACHIEVEMENTS; }
export function checkAchievements(stats: GameStats, currentProgress: AchievementProgress[]): AchievementProgress[] {
  ...
  const checks: [AchievementId, boolean][] = [
    [AchievementId.FIRST_WIN, stats.totalWins >= 1], [AchievementId.STREAK_5, stats.maxStreak >= 5],
    [AchievementId.STREAK_10, stats.maxStreak >= 10], [AchievementId.PERFECT_GAME, stats.perfectGames >= 1],
    [AchievementId.SPEED_DEMON, stats.fastCompletions >= 1], [AchievementId.PERSISTENCE, stats.totalGamesPlayed >= 50],
    [AchievementId.LEVEL_5, stats.highestLevel >= 5], [AchievementId.LEVEL_10, stats.highestLevel >= 10],
    [AchievementId.HIGH_SCORER, stats.highestScore >= 1000], [AchievementId.NO_MISTAKES, stats.perfectGames >= 1],
  ];
  return checks.map(([id, condition]) => {
    if (unlocked.has(id)) return currentProgress.find((p) => p.id === id)!;
