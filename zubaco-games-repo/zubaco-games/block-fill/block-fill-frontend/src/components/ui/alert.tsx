...
export function Alert({
  ...
  const closeAfterAnimationRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const handleClose = useCallback(() => {
    ...
    if (closeAfterAnimationRef.current) {
      clearTimeout(closeAfterAnimationRef.current);
    }
    closeAfterAnimationRef.current = setTimeout(() => {
      onClose?.();
      closeAfterAnimationRef.current = null;
    }, 300);
  }, [onClose]);
  ...
  }, [autoCloseMs, onClose, handleClose]);
  ...
  return (
    <div
      className={`flex items-center gap-3.5 rounded-xl px-4 py-3.5 w-full max-w-[520px] mx-auto transition-all duration-300 ease-out transform ${
