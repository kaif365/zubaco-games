...
-- Drop the old unique constraint on stage_id (allows only one row per stageId total)
ALTER TABLE "stage_configs" DROP CONSTRAINT IF EXISTS "stage_configs_stage_id_key";

-- Drop old index if it existed under a different name
...
-- Regular index for general stageId lookups (including soft-deleted rows)
