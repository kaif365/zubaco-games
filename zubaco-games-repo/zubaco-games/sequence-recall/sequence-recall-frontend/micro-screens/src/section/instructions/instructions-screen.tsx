...
type StageContent = StageInstructionContent;
...
const getStageContent = (
  ...
  const i18nRaw = i18n.t(`instructions.stages.${String(stage)}`, {
    returnObjects: true,
  });

  if (typeof i18nRaw === 'string') {
    return INSTRUCTION_SCREEN_CONTENT_MAP[stage];
  }

  const i18nContent = i18nRaw as Partial<StageContent>;

  return {
    ...INSTRUCTION_SCREEN_CONTENT_MAP[stage],
    ...i18nContent,
    slides:
      (i18nContent.slides as StageContent['slides'] | undefined) ??
      INSTRUCTION_SCREEN_CONTENT_MAP[stage].slides,
  };
};
...
export function GameInstructionsScreen({
  ...
  const content = useMemo(() => {
    const base = getStageContent(stage, i18n, contentByStage);
