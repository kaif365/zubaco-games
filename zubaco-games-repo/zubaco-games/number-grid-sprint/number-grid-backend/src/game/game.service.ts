...
import { generateGrid } from './engine/gridGenerator';
import { calculateScore, type CellAnswer, type StageConfig } from './engine/scorer';
import { analyzeInputTiming } from './engine/timingAnalyzer';
import { getLevelConfig } from './engine/levelConfig';
import * as crypto from 'crypto';

const DEFAULT_CONFIG: StageConfig = { gridSize: 6, revealDurationMs: 1000, hideIntervalMs: 3000, timeLimitMs: 120000, pointsPerCorrect: 10, timeBonusMultiplier: 2 };
...
export class GameService {
  ...
  async startGame(playerId: string, stageId: string, level?: number) {
    ...
    const config: StageConfig = dbConfig ? { gridSize: dbConfig.gridSize, revealDurationMs: dbConfig.revealDurationMs, hideIntervalMs: dbConfig.hideIntervalMs, timeLimitMs: dbConfig.timeLimitMs, pointsPerCorrect: dbConfig.pointsPerCorrect, timeBonusMultiplier: dbConfig.timeBonusMultiplier } : DEFAULT_CONFIG;

    const levelParams = level ? getLevelConfig(level) : null;
    if (levelParams) {
      config.timeLimitMs = levelParams.timeLimitMs;
      config.gridSize = levelParams.gridSize;
    }
    ...
    return { gameSessionId: session.id, endTime: new Date(Date.now() + config.timeLimitMs + 5000).toISOString(), serverTime: new Date().toISOString(), config, seed, level: levelParams?.level ?? 1, scoreMultiplier: levelParams?.scoreMultiplier ?? 1.0 };
  }

  async submitGame(playerId: string, gameSessionId: string, answers: CellAnswer[], clientScore: number) {
    ...
    // Calculate time remaining (estimate based on session timing)
    const elapsed = Date.now() - session.startedAt.getTime();
    const timeRemaining = Math.max(0, config.timeLimitMs - elapsed);

    const serverResult = calculateScore(answers, grid, timeRemaining, config);

    if (Math.abs(clientScore - serverResult.finalScore) > 5) {
    ...
    await this.prisma.input.createMany({ data: answers.map((a) => ({ gameSessionId, row: a.row, col: a.col, value: a.value, timestamp: BigInt(a.timestamp) })) });
    ...
    const { count } = await this.prisma.gameSession.updateMany({ where: { id: gameSessionId, status: 'active' }, data: { status, clientScore, serverScore: serverResult.finalScore, endedAt: new Date() } });
    ...
    return { finalScore: serverResult.finalScore, status, correctCells: serverResult.correctCells, totalCells: serverResult.totalCells, timeBonus: serverResult.timeBonus };
  }
