...
const STORAGE_KEY = 'number-grid-daily';
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm space-y-5">
        ...
        <div className="grid grid-cols-7 gap-2">
          {week.map((date, i) => {
            ...
            return (
              <div key={date} className={`flex flex-col items-center p-2 rounded-lg ${isToday ? 'border-2 border-blue-500 bg-blue-900/30' : 'bg-gray-800'}`}>
        ...
        <div className="bg-gray-800 rounded-xl px-4 py-3 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">Complete today's grid for bonus points</p>
        </div>
        ...
        <button onClick={onPlay} disabled={completedToday}
          className={`w-full py-3.5 rounded-xl font-bold text-lg shadow-lg transition-colors
            ${completedToday ? 'bg-gray-700 text-gray-400' : 'bg-blue-600 hover:bg-blue-500 text-white'}`}>
          {completedToday ? '✓ Completed Today' : 'Start Daily Grid'}
        </button>
      </motion.div>
    </div>
  );
