...
const STORAGE_KEY = 'speed-type-achievements';

const ACHIEVEMENT_DEFS: Omit<Achievement, 'unlocked'>[] = [
  { id: 'FIRST_GAME', title: 'First Game', description: 'Complete your first game', icon: '🎮' },
  { id: 'PERFECT_ROUND', title: 'Perfectionist', description: 'Get 100% accuracy in a game', icon: '💎' },
  { id: 'SPEED_DEMON', title: 'Speed Demon', description: 'Answer in under 2 seconds', icon: '⚡' },
...
export function checkAndUnlockAchievements(params: {
  ...
  const checks: [string, boolean][] = [
    ['FIRST_GAME', params.gamesPlayed >= 1],
    ['PERFECT_ROUND', params.accuracy >= 1.0],
    ['SPEED_DEMON', params.fastestMs > 0 && params.fastestMs < 2000],
    ['STREAK_5', params.streak >= 5],
    ['STREAK_10', params.streak >= 10],
    ['HIGH_SCORER', params.score >= 500],
    ['MARATHON', params.gamesPlayed >= 25],
