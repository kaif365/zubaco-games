export interface TimingConfig {
  /** Minimum average response time in ms (below this = bot) */
  minAvgResponseMs: number;
  /** Minimum single response time in ms */
  minSingleResponseMs: number;
  /** Maximum percentage of inputs below threshold before flagging */
...
/**
 * Analyze timing gaps between sequential inputs to detect bot-like behavior.
 * @param timestamps - Array of timestamps (ms) for each input, in order
 * @param config - Optional override for timing thresholds
 */
