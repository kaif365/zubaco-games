    ...
    >
      ...
      <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm space-y-3">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-green-400 font-semibold">{score.correctCount}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-400">Wrong</span>
          <span className="text-red-400 font-semibold">{score.wrongCount}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-400">Missed</span>
          <span className="text-yellow-400 font-semibold">{score.missedCount}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-400">Streak Bonus</span>
          <span className="text-purple-400 font-semibold">+{score.streakBonus}</span>
        </div>
      </div>

      <button
        onClick={onRestart}
        className="mt-4 px-8 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold text-lg transition-colors"
      >
        Play Again
      </button>
    </motion.div>
