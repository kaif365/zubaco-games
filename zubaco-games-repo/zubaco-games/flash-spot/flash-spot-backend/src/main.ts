...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Flash Spot API')
    .setDescription('ZUBACO Gaming Engine - Flash Spot Microservice')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  console.log(`Flash Spot backend running on port ${appConfig.port} | API: /api/v1`);
}
bootstrap();
