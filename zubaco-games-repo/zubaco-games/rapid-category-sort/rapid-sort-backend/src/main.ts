...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    .setTitle('Rapid Category Sort API')
    .setDescription('ZUBACO Gaming Engine - Rapid Category Sort Microservice')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  console.log(`Rapid Category Sort backend running on port ${appConfig.port} | API: /api/v1`);
}
bootstrap();
